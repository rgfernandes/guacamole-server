...
guac_palette* create_palette_orig(int height, int width, unsigned char *data, guac_palette* palette, int stride)
{
    int x, y;
    for (y=0; y<height; y++) {
        for (x=0; x<width; x++) {
...
guac_palette* create_palette_new(int height, int width, unsigned char *data, guac_palette* palette, int stride)
{
    int x, y;
...
guac_palette* guac_palette_alloc(cairo_surface_t* surface) {
    ...
    create_palette_orig(height, width, data, palette, stride);
    memset(palette, 0, sizeof(guac_palette));
    palette = create_palette_new(height, width, data, palette, stride);
